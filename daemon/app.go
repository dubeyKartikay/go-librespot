...
func (app *App) withCredentials(ctx context.Context, creds any) (err error) {
	return app.withAppPlayer(ctx, func(ctx context.Context) (*AppPlayer, error) {
		if len(app.state.Credentials.Data) > 0 {
			appPlayer, err := app.newAppPlayer(ctx, session.StoredCredentials{
				Username: app.state.Credentials.Username,
				Data:     app.state.Credentials.Data,
			})
			if err == nil {
			...
			if !retryDeviceAuthorization(creds, err) {
				return nil, err
			}
			app.log.Warnf("saved credentials were rejected; requesting a new device authorization code")
		}
		{
...
func retryDeviceAuthorization(creds any, err error) bool {
	if _, ok := creds.(session.DeviceAuthCredentials); !ok {
		return false
	}
	var loginErr *login5.LoginError
	return errors.As(err, &loginErr) && loginErr.Code == login5pb.LoginError_INVALID_CREDENTIALS
}
